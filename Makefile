...
.PHONY: help setup dev backend frontend test lint e2e clean
...
test: ## Run backend tests and frontend typecheck
...
e2e: ## Browser end-to-end check (needs both servers running)
	cd web && node scripts/e2e-check.mjs

clean:
	rm -f server/coverse.db
